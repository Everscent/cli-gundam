 ...
 * テトリスリモート操作
 */
TetrisRemoteController::TetrisRemoteController()
{
	this->udpSocket = gcnew UdpClientSocket();
	this->udpSocket->Received += gcnew UdpEventHandler(this, &TetrisRemoteController::UdpClientSocketReceived);
	this->isRunning = false;
}
// ----------------------------------------------------------------------------------------------------

TetrisMino^ TetrisRemoteController::CreateMino(cli::array<unsigned char,1> ^bytes, Anaheim::Tetris::TetrisField ^field)
{
...
void TetrisRemoteController::UdpClientSocketReceived(System::Object ^sender, Anaheim::UdpSocket::UdpEventArgs ^e)
{
...
bool TetrisRemoteController::Start(IPEndPoint ^endPoint)
{
...
bool TetrisRemoteController::Stop()
{
	if (!this->isRunning) return false;
	this->isRunning = false;
...
bool TetrisRemoteController::Send(Anaheim::Tetris::TetrisField ^field, Anaheim::Tetris::TetrisMino ^mino, IPEndPoint ^endPoint)
{
	if (!this->isRunning) return false;
...
void TetrisRemoteController::OnReceived(Anaheim::Tetris::TetrisRemoteEventArgs ^e)
{
